...
class PlutoTestControlWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.ui.hSliderTorqTgtValue.valueChanged.connect(
            self._callback_test_torque_target_changed
        )
        self.ui.hSliderPosTgtValue.valueChanged.connect(
            self._callback_test_position_target_changed
        )
        self.ui.hSliderCtrlBndValue.valueChanged.connect(
            self._callback_test_ctrlbnd_target_changed
        )
        self.ui.hSliderCtrlGainValue.valueChanged.connect(
            self._callback_test_ctrlgain_target_changed
        )
    ...
    def update_ui(self):
        _nocontrol = not (
            self.ui.radioTorque.isChecked() or self.ui.radioPosition.isChecked()
        )
        ...
        if _nocontrol:
            ...
            _val = self._pos2tgt(
                slrrange, valrange, self.ui.hSliderTorqTgtValue.value()
            )
            _str += f" {_val:-3.1f}Nm"
            ...
            _val = self._pos2tgt(
                slrrange, valrange, self.ui.hSliderCtrlBndValue.value()
            )
            _str += f" {_val:-1.2f}"
    ...
    def _callback_test_ctrlgain_target_changed(self, event):
        ...
        self.pluto.set_control_gain(
            _off + (_scale * self.ui.hSliderCtrlGainValue.value()) / 255
        )
        self.update_ui()
    ...
    def get_torque_slider_value_ranges(self):
        return (
            ...
            (pdef.PlutoTargetRanges["TORQUE"][0], pdef.PlutoTargetRanges["TORQUE"][1]),
        )
...
if __name__ == "__main__":
    ...
    pdataview = PlutoTestControlWindow(
        plutodev=plutodev,
        mech="HOC",
        dataviewer=True,
