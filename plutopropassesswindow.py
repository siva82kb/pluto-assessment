...
class States(Enum):
    ...
    def haptic_demo_states():
        return [States.DEMO_WAIT, States.DEMO_MOVING, States.DEMO_HOLDING]
...
class PlutoPropAssessData:
    def __init__(self, assessinfo: dict):
        ...
        self._rawwriter: CSVWriter = CSVWriter(
            fname=self.rawfile, header=Proprioception.RAW_HEADER
        )
        self._summwriter: CSVWriter = CSVWriter(
            fname=self.summaryfile,
    ...
    def add_newdata(self, dt, pos, ctrl):
        ...
        self._buffer["vel"].append(
            (pos - self._buffer["pos"][-2]) / dt if len(self._buffer["pos"]) > 1 else 0
        )
    ...
    def set_sensedpostorq(self, success=True):
        """ """
        self._trialpostorq["sensedpos"] = (
            self.prom[1] if not success else float(np.mean(self._buffer["pos"]))
        )
    ...
    def _get_trial_details_line(self, demomode=False, state="Haptic Demo"):
        ...
        # Not in demo mode.
        _dispstr = [
            "Trial: " + f"{self.current_trial:3d} / {self.ntrials:3d}"
            if self.current_trial > 0
            else f"- NA -"
        ]
        _dispstr += [
            "Target: " + f"{self.current_target:5.1f}cm"
...
class PlutoPropAssessmentStateMachine:
    ...
    def _act_demo_move(self):
        # Set the target.
        _tgtdetails = self._compute_target_details(
            self._data.current_target, demomode=True
        )
    ...
    def _display_instruction(self):
        if self._state != States.REST:
            ...
            if self._data.demomode:
                ...
                self._instdisp.setText(
                    f"PLUTO button to start trial {self._data.current_trial + 1} / {len(self._data.targets)}."
                )
    ...
    def _define_me_some_lambdas(self):
        ...
        self.subj_in_target = (
            ...
            < Proprioception.TGT_ERR_TH
        )
        self.subj_near_start_position = (
            lambda: self._pluto.hocdisp - self._data.startpos
            < Proprioception.TGT_ERR_TH
        )
        self.subj_near_prom = (
            lambda: np.abs(self._pluto.hocdisp - self._data.prom[1])
            < Proprioception.TGT_ERR_TH
        )
    ...
    def subj_is_holding(self):
        ...
        _th = (
            Proprioception.VEL_HOC_THRESHOLD
    ...
    def _compute_target_details(self, tgtpos, demomode=False) -> dict:
        ...
        _strtt = 0
        _speed = (
            1 if not demomode else (2.0 + np.random.rand())
        ) * Proprioception.MOVE_SPEED
        _dur = float(np.abs(tgtpos - pdef.HOCScale * np.abs(_initpos)) / _speed)
        return {"target": _finalpos, "target0": _initpos, "t0": _strtt, "dur": _dur}
...
class PlutoPropAssessWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self._smachine = PlutoPropAssessmentStateMachine(
            plutodev=self._pluto, data=self.data, instdisp=self.ui.subjInst
        )
    ...
    def update_ui(self):
        ...
        if self._smachine.state == States.REST:
            ...
            _dispstr += self.data._get_trial_details_line(
                demomode=False, state="Haptic Demo"
            )
    ...
    def _update_graph(self):
        ...
        self.ui.currPosLine1.setData(
            [self.pluto.hocdisp, self.pluto.hocdisp], [-40, 15]
        )
    ...
    def _propassess_add_graph(self):
        ...
        self.ui.aromLine1 = pg.PlotDataItem(
            [self.data.arom[1], self.data.arom[1]], [-40, 15], pen=_redpendot
        )
        ...
        self.ui.promLine1 = pg.PlotDataItem(
            [self.data.prom[1], self.data.prom[1]], [-40, 15], pen=_blupendot
        )
    ...
    def _callback_pluto_newdata(self):
        ...
        self._smachine.run_statemachine(PlEvnts.NEWDATA, _delt)
        # Update UI at 20Hz
    ...
    def _callback_pluto_btn_released(self):
        # Run the statemachine
        _action = self._smachine.run_statemachine(PlEvnts.RELEASED, self.pluto.delt())
        self.update_ui()
