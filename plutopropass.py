...
class PlutoPropAssesor(QtWidgets.QMainWindow, Ui_PlutoPropAssessor):
    ...
    def __init__(self, port, *args, **kwargs) -> None:
        ...
        self._smachines = {"calib": None, "rom": None, "prop": None}
    ...
    def _callback_select_subject(self):
        _subjid, _done = QInputDialog.getText(
            self, "Select Subject", "Enter subject ID:"
        )
    ...
    def _callback_calibrate(self):
        ...
        self._calibwnd = PlutoCalibrationWindow(
            plutodev=self.pluto, mechanism="HOC", modal=True
        )
    ...
    def _callback_assess_rom(self):
        # Disable main controls
        self._maindisable = True
        self._romwnd = PlutoRomAssessWindow(
            plutodev=self.pluto, mechanism="HOC", modal=True
        )
    ...
    def update_ui(self):
        ...
        if self._subjid is not None:
            if self._datadir is not None:
                self.pbSubject.setText(
                    f"Subject: {self._subjid} [{self._datadir.as_posix().split('/')[-1]}]"
                )
            else:
        ...
        if self._romdata["AROM"] > 0 and self._romdata["PROM"] > 0:
            self.pbRomAssess.setText(
                f"Assess ROM [AROM: {self._romdata['AROM']:5.2f}cm | PROM: {self._romdata['PROM']:5.2f}cm]"
            )
        else:
    ...
    def _get_curr_sess(self):
        return (
            self._subjdetails["type"][0].lower()
    ...
    def _open_devdata_viewer(self):
        self._devdatawnd = PlutoDataViewWindow(plutodev=self.pluto, pos=(50, 400))
